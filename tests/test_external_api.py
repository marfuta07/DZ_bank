...
@patch("utils.requests.get")
def test_get_exchange_rate_request_fails(mock_get):
...
@patch("utils.requests.get")
def test_get_exchange_rate_invalid_response(mock_get):
...
@patch("utils.requests.get")
def test_get_exchange_rate_non_200_status(mock_get):
...
def test_rub_to_rub_no_conversion_needed(mock_get_rate):
...
@patch("utils.get_exchange_rate")
def test_usd_to_rub_conversion_success(mock_get_rate):
...
@patch("utils.get_exchange_rate")
def test_eur_to_rub_conversion_success(mock_get_rate):
...
@patch("utils.get_exchange_rate")
def test_conversion_fails_returns_zero(mock_get_rate):
...
def test_unsupported_currency_returns_zero():
...
def test_none_transaction_returns_zero():
    """Проверка на None-вход."""
    result = convert_currency_to_rub(None)
    assert result == 0.0
